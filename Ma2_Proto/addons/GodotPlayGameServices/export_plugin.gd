...
const PLAY_GAMES_APP_ID := "291207727368"
...
class AndroidExportPlugin extends EditorExportPlugin:
	...
	func _get_android_manifest_application_element_contents(platform: EditorExportPlatform, debug: bool) -> String:
		...
		# Note: Play Games Services requires the definition of the `@string/game_services_project_id` resource
		# We force it's creation here instead of relying on manual setup via Godot Play Game Services because "android" folder is ignored in Git
		preload("res://addons/GodotPlayGameServices/dock.gd").write_res_file(PLAY_GAMES_APP_ID)
